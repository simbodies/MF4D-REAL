// Left Pressed event for button_gold
if (object_exists(obj_character_preview) && obj_character_preview.active_layer == "planet") {
    with (obj_character_preview) {
        ...
        if (index >= 0) {
            ...
            show_debug_message("Removing planet: " + string(Gold_planet));
        } else {
            // Add new planet with its scale to the list
            var planet_data = [Gold_planet, 0.46];
            ds_list_add(planet_list, planet_data);
            show_debug_message("Adding planet: " + string(Gold_planet) + " with scale: 0.46");
        }
