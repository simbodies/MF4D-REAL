// Left Pressed event for button_silver
if (object_exists(obj_character_preview) && obj_character_preview.active_layer == "planet") {
    with (obj_character_preview) {
        ...
        for (var i = 0; i < ds_list_size(planet_list); i++) {
            ...
            if (planet_data[0] == Silver_planet) {
                index = i;
                break;
            }
        }
        ...
        if (index >= 0) {
            ...
            show_debug_message("Removing planet: " + string(Silver_planet));
        } else {
            // Add new planet with its scale to the list
            var planet_data = [Silver_planet, 0.43];
            ds_list_add(planet_list, planet_data);
            show_debug_message("Adding planet: " + string(Silver_planet) + " with scale: 0.43");
        }
