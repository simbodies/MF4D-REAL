// Left Pressed event for button_diamond2
if (object_exists(obj_character_preview) && obj_character_preview.active_layer == "planet") {
    with (obj_character_preview) {
        ...
        for (var i = 0; i < ds_list_size(planet_list); i++) {
            ...
            if (planet_data[0] == Diamond_planet2) {
                index = i;
                break;
            }
        }
        ...
        if (index >= 0) {
            ...
            show_debug_message("Removing planet: " + string(Diamond_planet2));
        } else {
            // Add new planet with its scale to the list
            var planet_data = [Diamond_planet2, 0.45];
            ds_list_add(planet_list, planet_data);
            show_debug_message("Adding planet: " + string(Diamond_planet2) + " with scale: 0.45");
        }
